...
if (!empty($ride_id) && !empty($passenger_name) && !empty($passenger_phone)) {
    ...
    try {
        ...
        if ($stmt_check->rowCount() > 0) {
            ...
            // 2. Créer la réservation avec statut 'pending'
            $query_book = "INSERT INTO bookings (ride_id, passenger_name, passenger_phone, status) VALUES (:ride_id, :passenger_name, :passenger_phone, 'pending')";
            ...
            $conn->commit();
            echo json_encode(array("success" => true, "message" => "Demande de réservation envoyée. En attente de confirmation du conducteur."));
        } else {
            $conn->rollBack();
